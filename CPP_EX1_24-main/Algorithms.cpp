...
std::string Algorithms::shortestPath(Graph g, int src, int des) {
    ...
    dist[static_cast<size_t>(src)] = 0; // Convert to size_t
    ...
    while (!pq.empty()) {
        ...
        if (u == des) {
            std::string path = std::to_string(des);
            while (parent[static_cast<size_t>(des)] != -1) { // Convert to size_t
                path = std::to_string(parent[static_cast<size_t>(des)]) + " -> " + path; // Convert to size_t
                des = parent[static_cast<size_t>(des)]; // Convert to size_t
            }
            return path;
        }
